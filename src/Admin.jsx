    ...
    <div className="min-h-screen pb-28 font-sans">
      ...
      <main className="max-w-3xl mx-auto px-4 py-6">
        {activeTab === 'posts' && (
          <>
            <div className="mb-6">
              <h2 className="text-[28px] leading-tight font-extrabold text-stone-900 tracking-tight">{getGreeting()}</h2>
            ...
            <section className="grid grid-cols-3 gap-3 mb-6">
              <div className="bg-stone-100 rounded-3xl shadow-sm p-4">
                ...
                <p className="text-2xl font-extrabold text-stone-900">{summary.answered}</p>
              </div>
              <div className="bg-rose-50 rounded-3xl shadow-sm p-4">
                <p className="text-xs text-rose-500 font-medium">This week</p>
                <p className="text-2xl font-extrabold text-rose-700">{summary.thisWeek}</p>
              </div>
              <div className="bg-amber-50 rounded-3xl shadow-sm p-4">
                <p className="text-xs text-amber-600 font-medium">Pending</p>
                <p className="text-2xl font-extrabold text-amber-700">{summary.pending}</p>
              </div>
            </section>

            <div className="mb-4">
              <input value={term} onChange={e=>setTerm(e.target.value)} className="border border-stone-300 rounded-xl px-3 py-2 w-full" placeholder="Search…" />
            </div>

            {isLoading && <div className="text-center text-stone-500 mb-4">Loading posts...</div>}
            {loadError && <div className="text-center text-rose-600 bg-rose-50 p-4 rounded-2xl mb-4">{loadError}</div>}
        ...
        {activeTab === 'logbook' && (
          <section className="space-y-2">
            ...
            <div className="rounded-3xl shadow-sm p-6 mb-4 bg-indigo-50 border border-indigo-100">
      ...
      {composeOpen && (
        <div className="fixed inset-0 z-30 bg-black/40 flex items-end sm:items-center justify-center" onClick={() => setComposeOpen(false)}>
          <div className="bg-white rounded-t-3xl sm:rounded-3xl w-full sm:max-w-lg p-6 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <button onClick={submitPost} className="mt-4 w-full px-4 py-2 rounded-xl bg-stone-900 hover:bg-stone-800 text-white font-semibold">Post</button>
          </div>
        </div>
      )}

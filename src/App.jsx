  ...
  return (
    <div className="min-h-screen pb-24">
      ...
      <main className="max-w-3xl mx-auto px-4 py-6">
          ...
          <>
            <section className="grid grid-cols-2 gap-3 mb-6">
              ...
              <div className="bg-white border border-zinc-200 rounded-2xl shadow-sm p-4">
                ...
                <p className="text-2xl font-semibold text-zinc-900">{stats.requests}</p>
              </div>
            </section>
            ...
            <section className="bg-white border border-zinc-200 rounded-2xl shadow-sm p-6 mb-6">
              ...
              <button onClick={submit} className="mt-4 w-full sm:w-auto px-4 py-2 rounded-xl bg-zinc-900 hover:bg-zinc-800 text-white font-medium">Post</button>
            </section>

            <section className="space-y-3">
              {isLoading && <div className="text-center text-zinc-500">Loading posts...</div>}
              {error && <div className="text-center text-rose-600 bg-rose-50 p-4 rounded-xl">{error}</div>}
              {!isLoading && !error && posts.length === 0 && (
                <div className="text-center text-zinc-500">No posts yet</div>
              )}
              ...
              {!isLoading && !error && posts.map((g) => {
                ...
                return (
                  <div key={g.id} className={`max-w-[92%] ${mine ? 'ml-auto' : 'mr-auto'}`}>
                    <div className={`bg-white border border-zinc-200 rounded-2xl shadow-sm p-4 ${priorityBorder[prio]} ${mine ? 'bg-zinc-50' : 'bg-white'}`}>
        ...
        {activeTab === 'logbook' && (
          <>
            <section className="rounded-2xl shadow-sm p-6 mb-6 bg-zinc-900 border border-zinc-700">
              <h2 className="text-base font-semibold text-zinc-100">Log Book</h2>
              <p className="text-sm text-zinc-400 mt-1">How many scans did you knock out?</p>
              <p className="text-5xl font-semibold text-zinc-50 mt-4 mb-4 tracking-tight">{totalScans}<span className="text-base font-normal text-zinc-500 ml-2">total logged</span></p>
              <div className="flex gap-2">
                <input
                  ...
                  placeholder="e.g. 12"
                  className="border border-zinc-700 bg-zinc-800 text-zinc-100 placeholder-zinc-500 rounded-xl px-3 py-2 flex-grow"
                />
                <button onClick={logScan} className="px-4 py-2 rounded-xl bg-zinc-100 hover:bg-white text-zinc-900 font-medium whitespace-nowrap">Log it</button>
              </div>
            </section>

            <section className="space-y-2">
              {scanLoading && <div className="text-center text-zinc-500">Loading log book...</div>}
              {!scanLoading && scanEntries.length === 0 && (
                <div className="text-center text-zinc-500">No scans logged yet</div>
              )}

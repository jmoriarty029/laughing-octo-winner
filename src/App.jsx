...
import {
  addDoc, collection, onSnapshot, query, serverTimestamp, updateDoc, doc, arrayUnion, deleteDoc
...
const USER_EMAIL = 'puchu@love-ledger.app'
const MY_ROLE = 'user'
...
export default function App() {
  ...
  const filteredPosts = useMemo(() => {
    ...
    return posts.filter(g => `${g.title||''} ${g.details||''}`.toLowerCase().includes(t));
  }, [posts, term])
  ...
  async function handleLogin() {
    setLoginError('');
    try {
      await signInWithEmailAndPassword(auth, USER_EMAIL, input);
      setInput('');
      // onAuthStateChanged will flip loggedIn to true
    } catch (err) {
    ...
    <div className="min-h-screen pb-28 font-sans bg-stone-50 dark:bg-stone-950">
      ...
      <main className="max-w-3xl mx-auto px-4 py-6">
        {activeTab === 'posts' && (
          <>
            <div className="mb-6">
              <h2 className="text-[28px] leading-tight font-extrabold text-stone-900 dark:text-stone-100 tracking-tight">{getGreeting()}, {MY_NAME}</h2>
            ...
            <section className="grid grid-cols-3 gap-3 mb-6">
              <div className="bg-stone-100 dark:bg-stone-900 rounded-3xl shadow-sm p-4">
                ...
                <p className="text-2xl font-extrabold text-stone-900 dark:text-stone-100">{stats.answered}</p>
              </div>
              <div className="bg-rose-50 dark:bg-rose-950/40 rounded-3xl shadow-sm p-4">
                ...
                <p className="text-2xl font-extrabold text-rose-700 dark:text-rose-300">{stats.thisWeek}</p>
              </div>
              <div className="bg-amber-50 dark:bg-amber-950/40 rounded-3xl shadow-sm p-4">
                ...
                <p className="text-2xl font-extrabold text-amber-700 dark:text-amber-300">{stats.pending}</p>
              </div>
            </section>
            ...
            <input
              ...
              className="border border-stone-300 dark:border-stone-700 bg-white dark:bg-stone-900 text-stone-900 dark:text-stone-100 rounded-xl px-3 py-2 w-full mb-4"
              placeholder="Search…"
            />

            <section className="space-y-3">
              {isLoading && <div className="text-center text-stone-500 dark:text-stone-400">Loading posts...</div>}
              {error && <div className="text-center text-rose-600 dark:text-rose-400 bg-rose-50 dark:bg-rose-950/40 p-4 rounded-2xl">{error}</div>}
              {!isLoading && !error && posts.length === 0 && (
                <div className="text-center text-stone-500 dark:text-stone-400">No posts yet</div>
              )}
              {!isLoading && !error && posts.length > 0 && filteredPosts.length === 0 && (
                <div className="text-center text-stone-500 dark:text-stone-400">No posts match your search</div>
              )}

              {!isLoading && !error && filteredPosts.filter(g => !(pendingDelete && pendingDelete.type==='post' && pendingDelete.id===g.id)).map((g) => {
        ...
        {activeTab === 'logbook' && (
          <>
            ...
            <section className="rounded-3xl shadow-sm p-6 mb-6 bg-indigo-50 dark:bg-indigo-950/40 border border-indigo-100 dark:border-indigo-900">
              <h2 className="text-base font-bold text-indigo-900 dark:text-indigo-200">Log Book</h2>
              <p className="text-sm text-indigo-400 dark:text-indigo-400 mt-1">How many scans did you knock out?</p>
            ...
            <h3 className="text-lg font-bold text-stone-900 dark:text-stone-100 mb-3">Log a new entry</h3>
            <section className="bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-800 rounded-3xl shadow-sm p-6 mb-6">
              <p className="text-sm text-stone-500 dark:text-stone-400 mb-2 text-center">How many scans?</p>
              <div
                className="flex items-center justify-center gap-6 py-2 select-none"
                ...
                onTouchEnd={(e)=>{
                  ...
                  const threshold = 30;
                  const n = Number(scanCount);
                  if (deltaX < -threshold) setScanCount(String(Math.min(10, n + 1)));
                  else if (deltaX > threshold) setScanCount(String(Math.max(1, n - 1)));
                }}
              >
                <button
                  onClick={() => setScanCount(String(Math.max(1, Number(scanCount) - 1)))}
                  className="text-2xl font-medium text-stone-300 dark:text-stone-600 w-10 text-center"
                >
                  {Number(scanCount) > 1 ? Number(scanCount) - 1 : ''}
                </button>
                <span className="text-6xl font-extrabold text-stone-900 dark:text-stone-100 w-20 text-center tabular-nums">{scanCount}</span>
                <button
                  onClick={() => setScanCount(String(Math.min(10, Number(scanCount) + 1)))}
                  className="text-2xl font-medium text-stone-300 dark:text-stone-600 w-10 text-center"
                >
                  {Number(scanCount) < 10 ? Number(scanCount) + 1 : ''}
                </button>
              </div>
              <p className="text-center text-xs text-stone-400 dark:text-stone-500 mb-3">Swipe left/right, or tap a number</p>

              <div className="mt-3">
                <p className="text-sm text-stone-500 dark:text-stone-400 mb-2">Scan type <span className="text-rose-500">*</span></p>
                <div className="flex flex-wrap gap-2">
                  {SCAN_TYPES.map(t => (
                    <label key={t} className={`text-sm px-3 py-1.5 rounded-full border cursor-pointer font-medium ${scanType===t ? 'bg-stone-900 dark:bg-stone-100 text-white dark:text-stone-900 border-stone-900 dark:border-stone-100' : 'bg-white dark:bg-stone-800 text-stone-600 dark:text-stone-300 border-stone-300 dark:border-stone-700'}`}>
              ...
              {scanType === 'Others' && (
                <input
                  ...
                  className="border border-stone-300 dark:border-stone-700 bg-white dark:bg-stone-800 text-stone-900 dark:text-stone-100 rounded-xl px-3 py-2 w-full mt-3"
                />
              )}
              ...
              <button onClick={logScan} className="mt-4 w-full px-4 py-2 rounded-xl bg-indigo-900 dark:bg-indigo-700 hover:bg-indigo-800 dark:hover:bg-indigo-600 text-white font-semibold">Log it</button>
            </section>

            <section className="space-y-2">
              {scanLoading && <div className="text-center text-stone-500 dark:text-stone-400">Loading log book...</div>}
              {!scanLoading && scanEntries.length === 0 && (
                <div className="text-center text-stone-500 dark:text-stone-400">No scans logged yet</div>
              )}
              {!scanLoading && scanEntries.filter(e => !(pendingDelete && pendingDelete.type==='scan' && pendingDelete.id===e.id)).map((e) => (
      ...
      {composeOpen && (
        <div className="fixed inset-0 z-30 bg-black/40 flex items-end sm:items-center justify-center" onClick={() => setComposeOpen(false)}>
          <div className="bg-white dark:bg-stone-900 rounded-t-3xl sm:rounded-3xl w-full sm:max-w-lg p-6 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <button onClick={submit} className="mt-4 w-full px-4 py-2 rounded-xl bg-stone-900 dark:bg-stone-100 hover:bg-stone-800 dark:hover:bg-stone-200 text-white dark:text-stone-900 font-semibold">Post</button>
          </div>
        </div>
      )}

...
const scanTypeColors = {
  'X-ray': { bg: 'bg-sky-100', text: 'text-sky-700' },
  'USG': { bg: 'bg-teal-100', text: 'text-teal-700' },
  'MRI': { bg: 'bg-violet-100', text: 'text-violet-700' },
  'CT Scan': { bg: 'bg-amber-100', text: 'text-amber-700' },
  'Fluoroscopy': { bg: 'bg-fuchsia-100', text: 'text-fuchsia-700' },
  'Others': { bg: 'bg-stone-200', text: 'text-stone-700' },
  'Unspecified': { bg: 'bg-stone-200', text: 'text-stone-700' },
};
...
export default function App() {
  ...
  return (
    <div className="min-h-screen pb-28 font-sans">
      {pendingDelete && (
        <div className="fixed top-4 left-1/2 -translate-x-1/2 z-30 bg-stone-900 text-white rounded-full shadow-lg px-4 py-2 flex items-center gap-3 text-sm">
        ...
        {activeTab === 'posts' && (
          <>
            ...
            <section className="space-y-3">
              ...
              {!isLoading && !error && posts.filter(g => !(pendingDelete && pendingDelete.type==='post' && pendingDelete.id===g.id)).map((g) => {
        ...
        {activeTab === 'logbook' && (
          <>
            ...
            <section className="bg-white border border-stone-200 rounded-3xl shadow-sm p-6 mb-6">
              <p className="text-sm text-stone-500 mb-2 text-center">How many scans?</p>
              <div
              ...
              >
                <button
                  ...
                  className="text-2xl font-medium text-stone-300 w-10 text-center"
                >
                  {Number(scanCount) > 1 ? Number(scanCount) - 1 : ''}
                </button>
                <span className="text-6xl font-extrabold text-stone-900 w-20 text-center tabular-nums">{scanCount}</span>
                <button
              ...
              <p className="text-center text-xs text-stone-400 mb-3">Swipe left/right, or tap a number</p>
              ...
              {!scanLoading && scanEntries.filter(e => !(pendingDelete && pendingDelete.type==='scan' && pendingDelete.id===e.id)).map((e) => (
                <div key={e.id} className="bg-white border border-stone-200 rounded-2xl shadow-sm px-4 py-3">
                  <div className="flex items-center justify-between">
                    <span className="font-semibold text-stone-800">{pluralScans(e.count)} <span className="text-xs text-stone-400 ml-1 font-normal">({(e.authorRole||'user')===MY_ROLE ? 'Me' : 'Partner'})</span></span>
                  ...
                  {(e.authorRole||'user') === MY_ROLE && (
                    <div className="mt-2 text-right">
                      <button onClick={() => deleteScan(e.id)} className="text-xs text-stone-400 hover:text-rose-600 hover:underline">Delete</button>
                    </div>
                  )}
                </div>
              ))}
